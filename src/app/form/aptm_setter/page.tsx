...
interface FormData {
  priorExperience: string; solarExperience: string; commuteToNerang: string
  firstName: string; lastName: string; email: string; contactNumber: string
}

const questions = [
  { id: "priorExperience", type: "multiple", caption: "Please select the option that best describes your situation:", question: "Do you have prior appointment setting or admin experience?", options: ["Yes", "No"] },
  { id: "solarExperience", type: "multiple", caption: "Please select the option that best describes your situation:", question: "Do you have prior experience in the solar industry?", options: ["Yes", "No"] },
  { id: "commuteToNerang", type: "multiple", caption: "Please select the option that best describes your situation:", question: "Our office is located in Nerang. Are you able to reliably commute to this location?", options: ["Yes", "No"] },
...
const formUrl = "https://script.google.com/macros/s/AKfycbwdFFvLQBIzSdd0wayB51qWeu5du1d8DDYxwgUhEvEuKAj3hCAjt8VhzyhUjWK705dCUg/exec"

export default function AptmSetterFormPage() {
  const router = useRouter()
  const [currentStep, setCurrentStep] = useState(0)
  const [formData, setFormData] = useState<FormData>({ priorExperience: "", solarExperience: "", commuteToNerang: "", firstName: "", lastName: "", email: "", contactNumber: "" })
  ...
  const handleMultipleChoice = (value: string) => {
    ...
    if ((q.id === "priorExperience" || q.id === "solarExperience" || q.id === "commuteToNerang") && value === "No") {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const base = { ...formData, contactNumber: formatted, system_time: new Date().toISOString(), form_type: "appointment_setter", ...device }
        ...
        <Card className="w-full bg-white border-0">
          <CardContent className="p-8 text-center">
            ...
            {q.type === "phone" && (
              <div className="space-y-4">
                <div className="flex">
                  <span className="inline-flex items-center px-3 rounded-l-md border border-r-0 border-gray-300 bg-gray-100 text-black font-raleway">+61</span>
                  <Input placeholder={q.placeholder} value={formData.contactNumber}
                    ...
                    onKeyPress={handleKeyPress} name="contactNumber" className="rounded-l-none bg-gray-50 border-gray-300 text-lg text-black placeholder:text-gray-500 focus:border-[#002e6e] font-raleway" />
                </div>
