...
interface FormData {
  location: string; workType: string; position: string; cecAccreditation: string
  driversLicense: string; workingAtHeights: string; tools: string; startDate: string
...
const questions = [
  ...
  { id: "driversLicense", type: "multiple", caption: "Please select the option that best describes your situation:", question: "Do you hold a current drivers licence (manual)?", options: ["Yes", "No"] },
  { id: "workingAtHeights", type: "multiple", caption: "Can be obtained on a pay back system through company.", question: "Do you have a current Working at Heights Certification or will you be willing to obtain?", options: ["Yes", "No"] },
  { id: "tools", type: "multiple", caption: "Can be obtained on a pay back system through company.", question: "Do you have your own tools or a willingness to obtain them?", options: ["Yes", "No"] },
  { id: "startDate", type: "multiple", caption: "Please select the option that best describes your situation:", question: "If you're successful with this position when can you start?", options: ["Immediate", "1 Week", "2 Weeks", "3+ Weeks"] },
...
const formUrl = "https://script.google.com/macros/s/AKfycbxsgwvqJ-BkJS6aZsRZlGJpsClqLEtRgDmr99TRnbhj0oitkinSDPwJoLX19_P04BYjDg/exec"

export default function ElectricianFormPage() {
  const [currentStep, setCurrentStep] = useState(0)
  const [formData, setFormData] = useState<FormData>({ location: "", workType: "", position: "", cecAccreditation: "", driversLicense: "", workingAtHeights: "", tools: "", startDate: "", firstName: "", lastName: "", email: "", contactNumber: "" })
  ...
  const validateField = (field: string, value: string): string => {
    ...
    if (field === "contactNumber" && !/^\d{10}$/.test(value.replace(/\s/g, ""))) return "Please enter a valid 10-digit phone number"
  ...
  const handleMultipleChoice = (value: string) => {
    ...
    if ((q.id === "cecAccreditation" || q.id === "workingAtHeights" || q.id === "tools") && value === "No") {
      window.location.href = "/form/rejection"; return
    }
    setFormData((p) => ({ ...p, [q.id]: value }))
    setTimeout(() => { if (currentStep < questions.length - 1) setCurrentStep((p) => p + 1); else handleSubmit() }, 300)
  }
      ...
      <div className="w-full max-w-2xl">
        ...
        <Card className="w-full bg-white border-0">
          <CardContent className="p-8 text-center">
            ...
            {q.type === "text" && (
              <div className="space-y-4">
                <Input placeholder={q.placeholder} value={formData[q.id as keyof FormData]} onChange={(e) => handleTextInput(q.id, e.target.value)} onKeyPress={handleKeyPress} name={q.id} className="bg-gray-50 border-gray-300 text-black placeholder:text-gray-500 focus:border-[#002e6e] font-raleway text-lg" />
                {errors[q.id] && <p className="text-red-500 text-sm font-raleway">{errors[q.id]}</p>}
                <Button onClick={handleNext} disabled={isSubmitting} className="w-full bg-[#002e6e] text-white font-bold py-4 rounded-lg hover:bg-[#003a7a] font-raleway text-lg">
            ...
            {q.type === "name" && (
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Input placeholder="First name" value={formData.firstName} onChange={(e) => handleTextInput("firstName", e.target.value)} onKeyPress={handleKeyPress} name="firstName" className="bg-gray-50 border-gray-300 text-lg text-black placeholder:text-gray-500 focus:border-[#002e6e] font-raleway" />
